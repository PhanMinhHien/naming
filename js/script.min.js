...
function autoParseCampaignInfo() {
  var {
    ...
    campaign: a,
    campaignID: n,
  } = parseCampaignInfo(document.getElementById("campaignName").value);
  a && (document.getElementById("campaignName").value = a),
    n && (document.getElementById("campaignID").value = n),
    t && (document.getElementById("vendor").value = t),
    e &&
      (((a = document.getElementById("languageInput")).value = e),
...
function parseCampaignInfo(e) {
  ...
  if (
    (-1 !== n &&
      -1 !== l &&
      n < l &&
      ((e = e.slice(n + 1, l).join(" ")), (r = findBestMatchingVendor(e))),
    r ||
      ((n = findBestMatchingVendor(a)) &&
        ((r = n), console.log("Vendor found via fallback:", r))),
    r)
  ) {
    let t = r.replace(/[-_]/g, " ").trim().toLowerCase();
    var e = t.split(/\s+/),
      n = e[0],
      e = e.slice(1).join("[-\\s_]*"),
      e = e
        ? new RegExp(`\\b${n}(?:[-\\s_]*${e})?\\b`, "i")
        : new RegExp(`\\b${n}\\b`, "i"),
      n = a;
    (a = a
      .replace(e, "")
  ...
  i =
    -1 !== l
      ? (-1 !==
        (e = (n = a.split(/\s+/)).findIndex((e) => /^Q\d{3,4}$/i.test(e)))
          ? n.slice(e + 1)
          : n
        ).join(" ")
      : a;
...
function generateNames(e, t) {
  var a = parseCampaignInfo(
    document.getElementById("campaignName").value.replace(/\s+/g, "")
  );
  a.campaign && (document.getElementById("campaignName").value = a.campaign),
    a.vendor && (document.getElementById("vendor").value = a.vendor),
    a.langCode && (document.getElementById("languageInput").value = a.langCode);
