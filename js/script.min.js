...
vendorInput.addEventListener("input", function () {
  ...
  var e;
  t
    ? (e = vendorList.find((e) => e.toLowerCase() === t.toLowerCase()))
      ? ((this.value = e), (vendorListDiv.style.display = "none"))
      : renderVendorList(t)
    : (vendorListDiv.style.display = "none");
}),
  ...
  document.addEventListener("click", function (e) {
    vendorListDiv.contains(e.target) ||
      e.target === vendorInput ||
      (vendorListDiv.style.display = "none");
  });
...
function handleSingleSalesOrg(e) {
  ...
  return (
    ...
    renderTableByLanguageAndCode(e, t[0]),
    generateNames(e, t[0]),
    !0)
  );
}
    ...
    : ((salesOrgPopup.innerHTML = ""),
      e.forEach((e) => {
        ...
        (t.className = "autocomplete-item"),
          ...
          t.addEventListener("click", () => {
            (salesOrgPopup.style.display = "none"),
              renderTableByLanguageAndCode(a, e),
              generateNames(a, e);
          }),
  ...
  e.length <= 1
    ...
    : (e.forEach((e) => {
        ...
        (t.className = "autocomplete-item"),
          (t.textContent = e),
          t.addEventListener("click", () => {
            (languageInput.value = e),
              (languageList.style.display = "none"),
              handleSingleSalesOrg(e) || showSalesOrgPopup(e);
          }),
  ...
  e.length <= 1
    ...
    : (e.forEach((e) => {
        ...
        (t.className = "autocomplete-item"),
          (t.textContent = e),
          t.addEventListener("click", () => {
            (emailLanguageInput.value = e),
              (emailLanguageList.style.display = "none"),
              showEmailSalesOrgPopup(e);
          }),
...
emailLanguageInput.addEventListener("input", function () {
  ...
  e
    ? languageOptions.includes(e)
      ? ((emailLanguageInput.value = e),
        (emailLanguageList.style.display = "none"),
        handleSingleEmailSalesOrg(e) || showEmailSalesOrgPopup(e))
      : renderEmailLanguageList(e)
    : ((emailLanguageList.style.display = "none"),
      (emailSalesOrgPopup.style.display = "none"));
}),
  ...
  document.addEventListener("click", (e) => {
    languageList.contains(e.target) ||
      e.target === languageInput ||
      (languageList.style.display = "none"),
      salesOrgPopup.contains(e.target) ||
        e.target === languageInput ||
        (salesOrgPopup.style.display = "none");
  });
let dataMap = {
...
for (let a in dataMap)
  for (let e in dataMap[a]) {
    let t = dataMap[a][e];
    if (t.ctaText) {
      let e = t.ctaText.toLowerCase();
      t.ctaText = e.charAt(0).toUpperCase() + e.slice(1);
    }
    if (t.ctaText2) {
      let e = t.ctaText2.toLowerCase();
      t.ctaText2 = e.charAt(0).toUpperCase() + e.slice(1);
    }
  }
  ...
  Object.values(dataMap).forEach((e) => {
    Object.values(e).forEach((e) => {
      e.ctaText && (e.ctaText = capitalizeCTA(e.ctaText)),
        e.ctaText2 && (e.ctaText2 = capitalizeCTA(e.ctaText2));
    });
  });
...
function renderTableByLanguage(a) {
  ...
  e.forEach((e) => {
    ...
    l
      ? ((tableBody.innerHTML = o), (resultTable.style.display = "table"))
      : ((tableBody.innerHTML = ""), (resultTable.style.display = "none"));
}
...
function checkAndGenerate() {
  var e = document.getElementById("languageInput"),
    ...
    c = s && languageOptions.includes(s),
    d = ["vendor", "campaignName", "programCode", "customDate"].every((e) => {
      e = document.getElementById(e);
      return e && "" !== e.value.trim();
    });
  (n.style.display = "" !== r && "" !== t && c ? "block" : "none"),
    (i.style.display = l && "" !== l.value.trim() && c ? "block" : "none"),
    (o.style.display = a ? "block" : "none"),
    d
      ? (e.classList.remove("disabled"),
        (e.disabled = !1),
        c &&
          1 === (n = salesOrgMap[s] || []).length &&
          (renderTableByLanguageAndCode(s, n[0]), generateNames(s, n[0])))
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    document
      .getElementById("proofNameResult")
      .addEventListener("click", function (e) {
    ...
    document
      .getElementById("bannerPositionResult")
      .addEventListener("click", function (e) {
        let t = e.target.closest(".copy-text");
        t &&
          ((e = t.textContent.trim()),
  ...
  document.addEventListener("DOMContentLoaded", () => {
    [
      ...
      e = document.getElementById(e);
      e &&
        (e.addEventListener("input", checkAndGenerate),
        e.addEventListener("change", checkAndGenerate));
    }),
      checkAndGenerate();
  });
...
function generateProofName(e, t) {
  var a = document.getElementById("vendor").value.toLowerCase(),
    n = document
      ...
      .value.trim()
      .replace(/\s+/g, "_"),
    o = document.getElementById("idTicket").value.trim(),
    l =
    ...
    e = getWeekNumber(l);
  typeBanner
    ? ((l = `${t}_W${e}_${typeBanner}_${a}_${n}_` + o),
      ...
      document.getElementById("typeBanner").addEventListener("change", () => {
        var e,
          t = document
            ...
            .toUpperCase();
        t &&
          languageOptions.includes(t) &&
          0 < (e = salesOrgMap[t] || []).length &&
          generateProofName(t, e[0]);
      }))
    : (document.getElementById("proofNameResult").innerHTML = "");
}
function generatePositionBannerName(e, t) {
  var a = document.getElementById("vendor").value.toLowerCase(),
    ...
    t =
      (t || (salesOrgMap[e] && salesOrgMap[e][0]) || "0000") +
      `_${a}_${i}_${e}${n}_${o}_${l}_` +
      r;
...
function autoParseCampaignInfo() {
  var e,
    t,
    a = document.getElementById("campaignName").value.trim();
  console.log("raw", a),
    a &&
      (a.startsWith("Today:")
        ? ((t = parseTodayInfo(a)).campaign &&
            (document.getElementById("campaignName").value = t.campaign),
          t.campaignID &&
            (document.getElementById("campaignID").value = t.campaignID),
          t.vendor && (document.getElementById("vendor").value = t.vendor),
          t.langCode &&
            (((e = document.getElementById("languageInput")).value =
              t.langCode),
            e.dispatchEvent(new Event("input", { bubbles: !0 }))),
          t.programCode &&
            (document.getElementById("programCode").value = t.programCode),
          t.date &&
            (((e = document.getElementById("customDate")).value = new Date(
              t.date
            )
              .toISOString()
              .split("T")[0]),
            e.dispatchEvent(new Event("input"))),
          (t = document.getElementById("todayName")) && (t.value = a))
        : ((e = parseCampaignInfo(a)).campaign &&
            (document.getElementById("campaignName").value = e.campaign),
          e.campaignID &&
            (document.getElementById("campaignID").value = e.campaignID),
          e.vendor && (document.getElementById("vendor").value = e.vendor),
          e.langCode &&
            (((t = document.getElementById("languageInput")).value =
              e.langCode),
            t.dispatchEvent(new Event("input", { bubbles: !0 })))));
}
function parseTodayInfo(e) {
  var t = e.match(/\d{4}-\d{2}-\d{2}/),
    t = (t || new Date().toISOString().split("T"))[0],
    a = e.match(/([A-Z0-9]+)[\s-]*\d{4}-\d{2}-\d{2}/i),
    a = a ? a[1].trim() : "",
    e = e.replace("Today:", "").trim().split("-"),
    n = e[0],
    o = e[1],
    e = e.find((e) => /vendor/i.test(e)) || e[4] || "",
    l = getLangFromSalesOrg(n);
  return (
    console.log("langToken:", n),
    console.log("programCode:", o),
...
function findBestMatchingVendor(e) {
  ...
  var o,
    ...
    r = t[0].original;
  renderVendorList(r);
  for ([o, l] of reverseVendorMap.entries())
    if (l.toLowerCase() === r.toLowerCase()) return o;
...
function findQuarterToken(a) {
  ...
  for (let t = 0; t < a.length; t++)
    if (e.some((e) => e.test(a[t].toUpperCase()))) return t;
  return -1;
}
function parseCampaignInfo(e) {
  var t = (e = e.replace(
    ...
    t = t ? t[1].trim() : "";
  let a = e.split("-").slice(1).join("-").split("*")[0].trim(),
    n =
      ((a = a
        .replace(/[-_]/g, " ")
        .replace(
          /\b(webshop banner|megabanner|frontbanner|front banner)\b/gi,
          ""
        )
        ...
        .replace(/\s{2,}/g, " ")
        .trim()),
      console.log("🧹 Main part after cleaning:", a),
      a.split(/\s+/));
  var e = n.find((e) => /^\d{4}$/.test(e)),
    o = getLangFromSalesOrg(e),
    e = n.indexOf(e),
    l = findQuarterToken(n);
  let r = "",
    i = "";
  -1 !== e &&
    -1 !== l &&
    e < l &&
    ((l = n.slice(e + 1, l).join(" ")), (r = findBestMatchingVendor(l))),
    r ||
      ((l = findBestMatchingVendor(a)) &&
        ((r = l), console.log("🔍 Fallback vendor found:", r))),
    r &&
      ((l = new RegExp(`\\b${r}\\b`, "gi")),
      (l = (a = a.replace(l, "").trim()).split(/\s+/)),
      (a = l
        .filter((e) => !isCloseMatch(e.toLowerCase(), r.toLowerCase(), 0.5))
        .join(" ")
        .replace(/\s{2,}/g, " ")
        .trim()),
      console.log("🧹 Main part after removing vendor:", a));
  l = findQuarterToken((n = a.split(/\s+/)));
  i = (-1 !== l ? n.slice(l + 1) : n.slice(-1 !== e ? e + 1 : 0)).join(" ");
  let s = r.toLowerCase();
  return (
    (i = i
      .split(/\s+/)
      ...
      .join(" ")
      .trim()),
    console.log("🎯 Final:", { vendor: r, campaign: i }),
    { langCode: o || "", vendor: r, campaign: i, campaignID: t }
  );
}
function isCloseMatch(t, a, e = 0.85) {
  (t = t.toLowerCase()), (a = a.toLowerCase());
  var n = Math.max(t.length, a.length);
  if (0 === n) return !1;
  let o = 0;
  for (let e = 0; e < Math.min(t.length, a.length); e++) t[e] === a[e] && o++;
  return e <= o / n;
}
function generateNames(e, t) {
  var a = document.getElementById("campaignName").value.replace(/\s+/g, "");
  let n,
    o =
      ((n = (a.startsWith("Today:") ? parseTodayInfo : parseCampaignInfo)(a))
        .campaignID &&
        (document.getElementById("campaignID").value = n.campaignID),
      n.campaign &&
        (document.getElementById("campaignName").value = n.campaign),
      n.vendor && (document.getElementById("vendor").value = n.vendor),
      n.langCode &&
        (document.getElementById("languageInput").value = n.langCode),
      n.date &&
        ((a = new Date(n.date).toISOString().split("T")[0]),
        ((c = document.getElementById("customDate")).value = a),
        (a = new Event("change")),
        c.dispatchEvent(a)),
      document.getElementById("vendor").value.toLowerCase()),
    l = document.getElementById("campaignName").value.replace(/\s+/g, ""),
  ...
  var c = document.getElementById("customDate").value,
    a = c ? new Date(c) : new Date(),
    c = a.getFullYear().toString().slice(-2);
  let d =
      "" +
      String(a.getMonth() + 1).padStart(2, "0") +
      String(a.getDate()).padStart(2, "0"),
    m = c + "CW" + getWeekNumber(a),
    u = document.getElementById("tableBodyName");
  u.innerHTML = "";
  c = salesOrgMap[r] || [];
  c.forEach((t) => {
    ...
    generateProofName(r, c[0]),
    generatePositionBannerName(r, c[0]);
}
function getWeekNumber(e) {
  (e = new Date(
    Date.UTC(e.getFullYear(), e.getMonth(), e.getDate())
  )).setUTCDate(e.getUTCDate() + 4 - (e.getUTCDay() || 7));
...
document.getElementById("campaignName").addEventListener("input", (e) => {
  autoParseCampaignInfo(e.target.value);
}),
  document
  ...
  document.getElementById("bannerSize").addEventListener("change", () => {
    ...
    t &&
      languageOptions.includes(t) &&
      0 < (e = salesOrgMap[t] || []).length &&
      generatePositionBannerName(t, e[0]);
  }),
  document.addEventListener("DOMContentLoaded", () => {
    var e = new Date().toISOString().split("T")[0],
      e =
        ((document.getElementById("customDate").value = e),
        document.getElementById("typeBanner"));
    e &&
